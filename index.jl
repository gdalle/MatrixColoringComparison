...
function plot_comparison(data; matrix::Symbol)
	...
	ax = Axis(
		...
		ylabel="speedup SMC / ColPack $(matrix == :Jacobian ? "[log]" : "")",
		xscale=log10, yscale=(matrix == :Jacobian ? log10 : identity),
	)
	hlines!(ax, [1.0], linewidth=2, color=:black)
	for (i, d) in enumerate(sort(unique(data[!, :d])))
		...
		scatterlines!(
			...
			label="$d", linestyle=linestyles[i], marker=markers[i],
			markersize=12,
		)
	end
	axislegend(
		ax, "nonzeros\nper\ncolumn";
		position=(matrix == :Jacobian ? :lt : :lb),
	)
	return fig
end
...
data_jacobian = benchmark_coloring(
	n_values = floor.(Int, 10 .^ (1:0.3:2.5)),
	matrix=:Jacobian,
)
...
manifest_format = "2.0"
project_hash = "4f5f0485e5c0b0b8ff7e0c9097d7245f585262ea"
...
git-tree-sha1 = "18c1ebcf19232a0e51cf8c6260d371f3ce0def22"
